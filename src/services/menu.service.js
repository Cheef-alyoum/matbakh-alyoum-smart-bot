...
const ROOT_ALIASES = {
  cooked: 'bundles', bundles: 'bundles', bundle: 'bundles', chicken: 'bundles',
  main_chicken: 'bundles', meat: 'bundles', main_meat: 'bundles', mahashi: 'bundles',
  yalangi: 'bundles', maftoul: 'bundles', individual: 'bundles', salads: 'bundles',
  soups: 'bundles', fried: 'bundles', catering: 'catering', frozen: 'frozen'
};

const STATUS_LABELS = {
  ready: 'مطبوخ', raw: 'جاهز للطبخ', frozen: 'مفرز', made_to_order: 'حسب الطلب', bundle: 'عرض'
};
...
function safeString(value, fallback = '') { return (value == null ? '' : String(value).trim()) || fallback; }
...
function normalizeRootValue(item) {
  ...
  if (/خاروف|ضلعه|ضلعة|عزايم|ولائم/.test(category) || /خاروف|ضلعه|ضلعة|عزايم|ولائم/.test(section)) return 'catering';
  if (/مفرز/.test(normalizeArabic(item?.status || '')) || /مفرز/.test(category) || /مفرز/.test(section)) return 'frozen';
  return 'bundles';
}
...
function baseItemsForRoot(rootDir, rootId) {
  ...
  return loadMenu(rootDir).filter(isMenuItemVisible).filter(item => normalizeRootValue(item) === resolvedRoot);
}
...
function sortItemsForDisplay(items = []) {
  return [...items].sort((a, b) => {
    ...
    return safeString(a.display_name_ar || a.item_name_ar).localeCompare(safeString(b.display_name_ar || b.item_name_ar), 'ar');
  });
}

export function resolveRootId(rootId = '') { return cleanRootId(rootId); }
export function getRootDefinitions() { return ROOT_DEFINITIONS; }
export function getMenuData(rootDir) { return loadMenu(rootDir); }
export function getMetaCatalog(rootDir) { return safeArray(readJsonFile(path.join(rootDir, 'data', 'meta_catalog.json'), [])); }
export function getMenuSummary(rootDir) { return readJsonFile(path.join(rootDir, 'data', 'menu.summary.json'), {}); }
...
export function getSections(rootDir) {
  ...
  return [...map.entries()].map(([section_ar, sectionItems]) => ({
    section_ar, count: sectionItems.length, slug: slugify(section_ar), items: sortItemsForDisplay(sectionItems)
  }));
}
...
export function getItemsBySection(rootDir, sectionName) {
  ...
  return sortItemsForDisplay(loadMenu(rootDir).filter(item => isMenuItemVisible(item) && normalizeArabic(item.section_ar || '') === wanted));
}
...
export function getMenuItemById(rootDir, itemId) {
  ...
  return loadMenu(rootDir).find(item => {
    const values = [item.record_id, item.id, item.sku, item.display_name_ar, item.item_name_ar].map(value => safeString(value));
...
export function searchMenu(rootDir, q) {
  ...
  return sortItemsForDisplay(items.filter(item => {
    const haystack = normalizeArabic([item.record_id, item.sku, item.section_ar, item.item_name_ar, item.display_name_ar, item.category_ar, item.type_ar, item.unit_ar, item.notes_ar, item.status].filter(Boolean).join(' '));
...
export function getItemsForRoot(rootDir, filters = {}) {
  ...
  if (filterObject.categoryFilter) items = items.filter(item => categoryMatches(item, filterObject.categoryFilter));
  if (filterObject.meatType) items = items.filter(item => typeMatches(item, filterObject.meatType));
  if (filterObject.statusFilter) items = items.filter(item => statusMatches(item, filterObject.statusFilter));
  return sortItemsForDisplay(items);
}

export function getBotRoots(rootDir) {
  return ROOT_DEFINITIONS.map(root => ({ ...root, count: baseItemsForRoot(rootDir, root.id).length })).filter(root => root.count > 0);
}
...
export function getRootCategoryOptions(rootDir, rootId, filters = {}) {
  ...
  const values = uniqueBy(items.filter(item => safeString(item.category_ar)), item => normalizeArabic(item.category_ar)).map(item => item.category_ar);
  return sortByReference(values, CATEGORY_ORDER[resolved] || []).map(value => ({
    value, label: value, slug: slugify(value), count: items.filter(candidate => normalizeArabic(candidate.category_ar) === normalizeArabic(value)).length
  }));
}
...
export function getRootTypeOptions(rootDir, rootId, filters = {}) {
  ...
  const values = uniqueBy(items.filter(item => safeString(item.type_ar)), item => normalizeArabic(item.type_ar)).map(item => item.type_ar);
  return sortByReference(values, TYPE_ORDER).map(value => ({
    value, label: value, slug: slugify(value), count: items.filter(candidate => normalizeArabic(candidate.type_ar) === normalizeArabic(value)).length
  }));
}

export function getRootStatusOptions(rootDir, rootId) {
  const items = baseItemsForRoot(rootDir, rootId);
  const statuses = uniqueBy(items.map(item => safeString(item.status)).filter(Boolean), value => normalizeArabic(value));
  return statuses.map(status => ({
    value: status, label: STATUS_LABELS[status] || status, slug: slugify(status), count: items.filter(item => normalizeArabic(item.status || '') === normalizeArabic(status)).length
  }));
}
...
export function getItemExtras(rootDir, item) {
  ...
  if (/مسخن/.test(title)) {
    const bread = items.find(candidate => /رغيف مسخن إضافي|رغيف مسخن اضافي/.test(`${safeString(candidate.display_name_ar)} ${safeString(candidate.item_name_ar)}`));
    if (bread) extras.push({ id: bread.record_id, label: bread.display_name_ar || bread.item_name_ar, price: Number(bread.price_1_jod || 0) });
  }

  if (/مفتول/.test(title)) {
    const vegetables = items.find(candidate => /إضافة خضروات للمفتول|اضافه خضروات للمفتول/.test(`${safeString(candidate.display_name_ar)} ${safeString(candidate.item_name_ar)}`));
    if (vegetables) extras.push({ id: vegetables.record_id, label: vegetables.display_name_ar || vegetables.item_name_ar, price: Number(vegetables.price_1_jod || 0) });
  }
