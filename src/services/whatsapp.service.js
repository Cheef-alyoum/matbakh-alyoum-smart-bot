...
function adminHomeButtons(profile = null) {
  ...
  if (profileCan(profile, 'admin.orders.view')) {
    buttons.push({ id: BUTTON_IDS.ADMIN_ORDERS, title: 'الطلبات' });
  }
  if (profileCan(profile, 'admin.reports.view')) {
    buttons.push({ id: BUTTON_IDS.ADMIN_REPORTS, title: 'التقارير' });
  }
  if (profileCan(profile, 'admin.campaigns.view')) {
    buttons.push({ id: BUTTON_IDS.ADMIN_CAMPAIGNS, title: 'الحملات' });
  }
...
async function handleAdminAction(rootDir, from, selection, config) {
  ...
  if ([BUTTON_IDS.ADMIN_APPROVE, BUTTON_IDS.ADMIN_MODIFY, BUTTON_IDS.ADMIN_REJECT].includes(action)) {
    if (!profileCan(adminProfile, 'admin.orders.approve')) {
      return { ok: false, message: adminPermissionDenied(adminProfile, 'صلاحيات اعتماد الطلبات') };
    }
  }

  if ([BUTTON_IDS.ADMIN_PREPARING, BUTTON_IDS.ADMIN_READY, BUTTON_IDS.ADMIN_OUT, BUTTON_IDS.ADMIN_DELIVERED].includes(action)) {
    if (!profileCan(adminProfile, 'admin.orders.status')) {
      return { ok: false, message: adminPermissionDenied(adminProfile, 'صلاحيات تغيير حالة الطلب') };
    }
  }
...
export async function processWhatsAppWebhook(rootDir, req, res, config) {
  try {
    ...
    logWebhook('WEBHOOK_IN', {
      ...
      hasStatuses: Boolean(value.statuses?.[0]),
      from: message?.from || null,
      type: message?.type || null,
      messageId: message?.id || null,
      statusId: value.statuses?.[0]?.id || null,
      statusType: value.statuses?.[0]?.status || null
    });
    ...
    const adminProfile = isAdminAuthorized(rootDir, from, config)
      ? getAdminProfile(rootDir, from, config)
      : null;
    ...
    if (adminProfile && (type === 'interactive' || type === 'button')) {
      ...
      if (selection === BUTTON_IDS.ADMIN_ORDERS) {
        if (!profileCan(adminProfile, 'admin.orders.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'قسم الطلبات'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_orders_denied' });
        }
      ...
      if (selection === BUTTON_IDS.ADMIN_REPORTS) {
        if (!profileCan(adminProfile, 'admin.reports.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'قسم التقارير'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_reports_denied' });
        }
      ...
      if (selection === BUTTON_IDS.ADMIN_CAMPAIGNS) {
        if (!profileCan(adminProfile, 'admin.campaigns.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'قسم الحملات'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_campaigns_denied' });
        }
      ...
      if (selection === BUTTON_IDS.ADMIN_ORDERS_NEW) {
        if (!profileCan(adminProfile, 'admin.orders.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'عرض الطلبات الجديدة'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_orders_new_denied' });
        }
      ...
      if (selection === BUTTON_IDS.ADMIN_ORDERS_FOLLOW) {
        if (!profileCan(adminProfile, 'admin.orders.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'متابعة الطلبات'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_orders_follow_denied' });
        }
      ...
      if (selection === BUTTON_IDS.ADMIN_ORDERS_SEARCH) {
        if (!profileCan(adminProfile, 'admin.orders.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'البحث في الطلبات'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_orders_search_denied' });
        }
      ...
      if (selection.startsWith('admin_follow_status:')) {
        if (!profileCan(adminProfile, 'admin.orders.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'عرض حالات الطلبات'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_follow_denied' });
        }
      ...
      if (selection.startsWith('admin_view:')) {
        if (!profileCan(adminProfile, 'admin.orders.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'عرض تفاصيل الطلب'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_view_denied' });
        }
      ...
      if (selection === BUTTON_IDS.ADMIN_REPORT_TODAY) {
        if (!profileCan(adminProfile, 'admin.reports.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'تقرير اليوم'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_report_today_denied' });
        }
      ...
      if (selection === BUTTON_IDS.ADMIN_REPORT_WEEK) {
        if (!profileCan(adminProfile, 'admin.reports.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'تقرير الأسبوع'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_report_week_denied' });
        }
      ...
      if (selection === BUTTON_IDS.ADMIN_REPORT_MONTH) {
        if (!profileCan(adminProfile, 'admin.reports.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'تقرير الشهر'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_report_month_denied' });
        }
      ...
      if (selection === BUTTON_IDS.ADMIN_CAMPAIGN_NEW) {
        if (!profileCan(adminProfile, 'admin.campaigns.manage')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'إنشاء الحملات'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_campaign_new_denied' });
        }
      ...
      if (selection === BUTTON_IDS.ADMIN_CAMPAIGN_SCHEDULE) {
        if (!profileCan(adminProfile, 'admin.campaigns.manage')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'جدولة الحملات'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_campaign_schedule_denied' });
        }
      ...
      if (selection === BUTTON_IDS.ADMIN_CAMPAIGN_GROUPS) {
        if (!profileCan(adminProfile, 'admin.groups.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'عرض مجموعات الجمهور'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_groups_denied' });
        }
      ...
      if (selection.startsWith('admin_group:')) {
        if (!profileCan(adminProfile, 'admin.groups.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'معاينة مجموعات العملاء'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_group_preview_denied' });
        }
    ...
    if (adminProfile && type === 'text') {
      ...
      if (command === '/pending') {
        if (!profileCan(adminProfile, 'admin.orders.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'عرض الطلبات الجديدة'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_pending_denied' });
        }
        const delivered = await sendAdminPendingOrders(rootDir, to);
      ...
      if (command.startsWith('/view ')) {
        if (!profileCan(adminProfile, 'admin.orders.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'عرض الطلب'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_view_denied_text' });
        }
      ...
      if (/^(التقارير|تقارير)$/.test(normalizedAdminText)) {
        if (!profileCan(adminProfile, 'admin.reports.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'قسم التقارير'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_reports_denied_text' });
        }
        const delivered = await sendWhatsAppInteractive(rootDir, to, adminReportsButtons());
      ...
      if (/^(الطلبات|طلبات)$/.test(normalizedAdminText)) {
        if (!profileCan(adminProfile, 'admin.orders.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'قسم الطلبات'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_orders_denied_text' });
        }
        const delivered = await sendWhatsAppInteractive(rootDir, to, adminOrdersButtons());
      ...
      if (/^(الحملات|حملات|العروض|عروض)$/.test(normalizedAdminText)) {
        if (!profileCan(adminProfile, 'admin.campaigns.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'قسم الحملات'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_campaigns_denied_text' });
        }
        const delivered = await sendWhatsAppInteractive(rootDir, to, adminCampaignButtons());
      ...
      if (/^\/report-today$/i.test(command)) {
        if (!profileCan(adminProfile, 'admin.reports.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'تقرير اليوم'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_report_today_denied_text' });
        }
        const delivered = await sendAdminQuickReport(rootDir, to, 'اليوم', 'today');
      ...
      if (/^\/report-week$/i.test(command)) {
        if (!profileCan(adminProfile, 'admin.reports.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'تقرير الأسبوع'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_report_week_denied_text' });
        }
        const delivered = await sendAdminQuickReport(rootDir, to, 'الأسبوع', 'week');
      ...
      if (/^\/report-month$/i.test(command)) {
        if (!profileCan(adminProfile, 'admin.reports.view')) {
          const delivered = await sendWhatsAppText(rootDir, to, adminPermissionDenied(adminProfile, 'تقرير الشهر'));
          return json(res, 200, { ok: true, delivered, mode: 'admin_report_month_denied_text' });
        }
        const delivered = await sendAdminQuickReport(rootDir, to, 'الشهر', 'month');
